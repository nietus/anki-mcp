 ...
 */
async function findCardsAndOrder(client: YankiConnect, ankiQuery: string): Promise<Card[]> {
  ...
  const mappedCards: Card[] = cardsData.map((card: { cardId: number; question: string; answer: string; due: number; }) => ({
    cardId: card.cardId,
...
export function registerResourceHandlers(server: Server, getClient: () => YankiConnect) {
  /**
